...
class TwitterAuthorizationHeaderSpec extends FlatSpec with Matchers with OptionValues with Inside with Inspectors {
  ...
  val expectedSignature = "tnnArxj06cWHq44gCs1OSKk/jLY="
  ...
  "Twitter Auth Header" should "create a valid parameter string" in {
    val parameterString = _parameterString
    assert(parameterString.equals(expectedParameterString))
  }

  it should "create a valid signature base string" in {
  ...
  it should "create valid signing keys" in {
  ...
  it should "create valid authorization headers" in {
